...
class Principal(QMainWindow):
    ...
    def __init__(self):
        ...
        # boton de configuración de tema
        ...
        # cierre de sesión no importa quien este logeado
        self.ui.btn_salir.clicked.connect(self.Salir_al_inicio)

    #region Módulo inicial, poner las varibles en 0.
    def Iniciar_cero(self):
        """
        Método para inciar de cero las variables globales y los campos
        """
        global usuario
        global contra
        global codigo
    ...
    def Salir_al_inicio(self):
        """
        Permite cerrar sesión ya sea para el usuario o administrador
    ...
    def Ventana_reg_pac(self, num):
        ...
        if num == 2:
            reg_pac = 2
            self.ui.lbl_reg_mod_pac.setText("Edición del registro de historial de paciente")
    ...
    def Ventana_nueva_hist(self):
        ...
        if not nom_pac or grupo_pac==0 or factor_pac==0 or sexo_pac==0 or not procedencia or not residencia or not direccion or not barrio:
            ...
            QMessageBox.critical(self,"Mensaje de error","No se guardan sólo apellidos, debe tener un nombre")
        else:
                        ...
                        if resp2==1:
                            ...
                            if resp4==1:
                                ...
                                if resp5[0]==1:
                                    ...
                                    QMessageBox.critical(self,"Mensaje de error","No se pudo verificar el código de paciente")
                            else:
    ...
    def Verificar1(self,datos):
        ...
        bbdd_tel1 = ""
        bbdd_tel2 = ""
        QMessageBox.information(self, "Guardado", "Se ha modificado con éxito los datos mencionados")
    ...
    def Guardar_consulta(self):
        ...
        if combo==0:
            ...
            if not per_cef:
                per_cef=0.0
            if not motivo:
                motivo="no se registró el motivo"
            if not examen:
                examen="no se realizó el exámen físico"
            if not diagnos:
                diagnos="no se tiene un diagnóstico para registrar"
            if not trat:
                trat="no se tiene un tratamiento para registrar"
            if not obs:
                obs="no se tiene ningua observación para registrar"
            ...
            if resp == 1:
                ...
                if resp2 == 1:
                    ...
                    if resp3[0] == 1:
                        ...
                        QMessageBox.critical(self,"Mensaje de error","No se pudo verificar el código de la consulta")
                else:
    ...
    def Verifica(self):
        ...
        if not usuario or not contra:
            ...
            if resp==0:
                QMessageBox.critical(self, "Mensaje de advertencia", "El Usuario y/o contraseña no son correctos\n"
                ...
                if resp1[0]==1:
                    ...
                    if resp2[0]==0:
                        ...
                        if resultado[1]==False:
                            codigo=QInputDialog.getText(self,"Ventana para introducir su código","Ingrese el código enviado a su correo")
                            columna = "codigo"
                            tabla = "validaciones"
                            valor = "id_validaciones", resp3[1]
                            ...
                            if resp4[0]==1:
                                codigo1=resp4[1]
                                if codigo[0]!=codigo1:
                                    QMessageBox.critical(self,"Mensaje de error","El código ingresado no corresponde al enviado al correo, verifique nuevamente")
                                else:
                                    tabla="validaciones"
                                    columnas="validacion",
    ...
    #endregion
    # ------------------------------------------------------------------------------------------
    #region Dirige al módulo de entrada medico o admi
    ...
    def Verifica_new_user(self):
        ...
        if not carnet[0] or carnet[1]==0 or not medico[0] or not persona[1] or not usuario1[1] or not contra1 or not medico[3] or persona[5]==0:
            ...
            QMessageBox.critical(self, "Mensaje de error", "Las contraseñas deben ser iguales")
        elif self.Verifica_email(medico[3]) == 1:
            QMessageBox.critical(self, "Mensaje de advertencia", "Introduzca una dirección de correo válida\n"
            ...
            if reg_us==1:
                ...
                if resp9==1:
                    ...
                    if resp10==1:
                                        ...
                                        if resp4==1:
                                            ...
                                            if resp2==1:
                                                ...
                                                if resp6==1:
                                                    ...
                                                    if resp7==1:
                                                        QMessageBox.information(self,"Mensaje de éxito","Se ha registrado con éxito al sistema.\n"
                                                                                                "Revise su correo electrónico para revisar la clave que se le envió")
                                                        self.Iniciar_cero()
                                                    else:
                                                        QMessageBox.critical(self,"Mensaje de error","Ha ocurrido un error y no se pudo guardar su nueva contraseña")
                                                else:
                                                    QMessageBox.critical(self,"Mensaje de error","Ha ocurrido un error y no se pudo guardar los datos de médico")
                                            else:
                        ...
                        QMessageBox.critical(self, "Mensaje de error", "No se pudo conectar a internet o ocurrió algún error, verifique su conexión a internet")
                else:
    ...
    def Verifica_email(self,mail):
        ...
        return r
    #endregion
    #region metodo para verificar espacios en blanco para recuperar la contraseña
    def Verifica_recu(self):
        ...
        if not usuario or not mail:
            QMessageBox.critical(self, "Campos vacios", "Debe llenar todos los espacios\n"
                                                        "Introduzca su Usuario y correo con el que se registró")
        else:
            usuario=str(usuario)
            mail=str(mail)
            if self.Verifica_email(mail)== 1:
                ...
                if resp[0]!=1 or mail != resp[1]:
                    ...
                    if resp10 == 1:
                        ...
                        QMessageBox.information(self, "Código enviado", "Código enviado exitosamente, revisar su correo")
                        ...
                        QMessageBox.critical(self,"Mensaje de error de envío","Ocurrio un error y no se pudo enviar el código\n"
                                                                              "de validación a su correo")
    #endregion
    ...
    def Verifica_codigo(self):
        ...
        if not codigo1:
            ...
            if codigo1 != codigo:
                QMessageBox.critical(self, "Mensaje de error", "El código introducido no es correcto\n"
                                                               "Revise su correo")
            else:
                self.ui.Stacked_recu.setCurrentIndex(2)
                self.ui.txt_recu_contra1.setFocus()
    #endregion
    #region verifica espacios vacios y contraseña
    def Verifica_pass(self):
        ...
        if not contra1 or not contra2:
            QMessageBox.critical(self, "Campos vacíos", "Debe llenar los espacios en blanco")
        else:
            if contra1 != contra2:
                QMessageBox.critical(self, "Error de datos", "Verifique que sus contraseñas sean iguales")
            else:
                contra1=str(contra1)
                encriptado = Clases.Metodos.Encriptador2(2,contra1)
                tabla="usuario"
                ...
                if resp==1:
                    ...
                    if resp1==1:
                        ...
                        QMessageBox.critical(self,"Mensaje de error","Ha ocurrido un error y no se pudo guardar la nueva contraseña")
                else:
                    QMessageBox.critical(self,"Mensaje de error","Ha ocurrido algún error y no se pudo cambiar la contraseña")
    #endregion
    # -----------------------------------------------------------------------------------------
        ...
        if not user1:
            ...
            if resp==1:
                ...
                if reg_us_admin==2:
                    ...
                    if resp2 == 1:
                        QMessageBox.information(self, "Mensaje de éxito", "Se ha cambiado la contraseña con éxito\n"
                                                                          "Seleccione guardar o imprimir después de cerrar este díalogo")
                        texto1 = (("Usuario: ", user1), ("Contraseña: ", password1))
                        nombre1 = "Cambio de contraseña"
                        self.Enviar_datos(1, texto1, nombre1)
                        self.ui.txt_reg_user.clear()
                    else:
                        QMessageBox.critical(self, "Mensaje de error", "Algo salió mal, por favor intente nuevamente")
                        self.ui.txt_reg_user.clear()
                ...
                if reg_us_admin==1:
                    ...
                    if resp1 == 1:
                        QMessageBox.information(self, "Mensaje de éxito", "Usuario creado con éxito\n"
                                                                          "seleccione guardar o imprimir después de cerrar este díalogo")
                        texto = (("Usuario: ", user1), ("Contraseña: ", password))
    ...
    def Calcular_fecha(self, fecha1, fecha):
        ...
        #global y
        var = True
        #dentro del while se genera un bucle que permite contrar los años normales y
        #los años bisiestos, con esto podemos saber los dias exactos que tiene
    ...
    def Verifica_IMC(self):
        ...
        if not w1 and not altura:
            ...
            QMessageBox.critical(self, "Advertencia error", "No se puede realizar división entre cero\n"
    ...
    #endregion
    #region Aqui hacemos la lógica para el IMC
    ...
    def Combo_cambio1(self):
        ...
        if a==12:
            ...
            self.ui.cb_admin_med.addItem("Médicos")
            if not resultado:
                QMessageBox.critical(self,"Mensaje de error","No hay médicos registrados o no se pudo ingresar al a base de datos")
            else:
                c = 0
                for i in resultado:
                    medico.insert(c,i[0])
    ...
    def Ventana_admi_reg(self,caso):
        ...
        if caso==1:
            ...
            reg_us_admin=1
        if caso==2:
            self.ui.lbl_admin_reg_titulo.setText("Resetear la contraseña de usuario")
            reg_us_admin=2
    #endregion
    # ------------------------------------------------------------------------------------------
    ...
    # Limpia ventana de recuperacion de contraseña
    ...
    def Validar_entrada(self, valida, valida1, valida2, valida3, valida4, valida5,valida6):
        """
        Los valores de entrada vienen del método de expresiones regulares
    ...
    def Enviar_datos(self,caso,texto,nombre):
        if caso==1:
            ...
            <body>
            <h1>Sistema de Información de historiales Clínicos Pedíatricos<br/></h1>
            <h1>"Consultorio Pedíatrico Divino Niño Jesús"<br/></h1>
            <h2>Creación de Cuenta de Usuario<br/></h2>
              [DATOS]
            </body>
            </html>
            """
    ...
    def Crear_html(self,entrada):
        html = """
                    ...
                    <body>
                    <h1>Sistema de Información de historiales Clínicos Pedíatricos</h1>
                    <h1>"Consultorio Pediátrico Divino Niño Jesús"</h1>
                    <h2>Historial Clínico del paciente<br/></h2>
    ...
    def Seleccionar_item_tabla(self,caso):
                ...
                if seleccionada[5]=="Habilitado":
                    ...
                    if resp==1:
                        mensaje = "Se ha Habilitado al usuario: {} {} {}".format(seleccionada[1].upper(),
                                                                                          seleccionada[2].upper(),
                                                                                          seleccionada[3].upper())
                        QMessageBox.information(self,"Información de resultado",mensaje)
                    else:
                        QMessageBox.critical(self,"Mensaje de error","Ha ocurrido algún error y no se pudo realizar los cambios")
            ...
            if elegida:
                ...
                if x==1:
                    ...
                    if resp1[0] == 1:
                        ...
                        patologicos="No se encontró nada en la base de datos"
                        alergias = "No se encontró nada en la base de datos"
                        datos = (nombre, codigo, patologicos, alergias)
                        x = 0
                        self.ui.Stacked_main.setCurrentIndex(8)
                        self.Ventana_reg_hist2(datos)
        ...
        if caso==4:
            ...
            ex = ['CH', 'LP', 'CB', 'OR', 'PT', 'TJ', 'SC', 'BE', 'PD', 'Otro país']
            ...
            if elegida:
                ...
                if datos_paciente:
                            ...
                            if len(datos) > 1:
                                ...
                                if tipo_fam1 == 1:
                                    ...
                                    if resp7[0] == 1:
                                        ...
                                        tel2 = "No tiene número registrado"
                                else:
                                ...
                                if tipo_fam1 == 1:
                                    ...
                                    if resp6[0] == 1:
                                        ...
                                        tel2 = "No tiene número"
                                    tel1 = "No tiene número registrado"
                    enviar1 = "Código de paciente: {} ".format(codigo_paciente1)
                    ...
                    enviar9="Dirección: {}".format(direc)
                    ...
                    enviar16="Teléfono o celular: {}".format(tel1)
                    enviar17 = "Nombre de la madre: {}".format(nom_madre)
                    enviar18 = "CI: {} {}".format(carnet2,extension2)
                    enviar19="Teléfono o celular: {}".format(tel2)
                    enviar20="Antecedentes patológicos: {}".format(antecedentes)
        ...
        if caso==5:
            ...
            if elegida:
                ...
                if resp8[0]==1:
                    ...
                    enviar1="Médico encargado de la atención: {}".format(dr)
                    enviar2="Fecha y hora de atención: {}".format(fecha_ate)
                    ...
                    enviar6="Perímetro cefálico: {}[cm]".format(per_cef)
                    enviar7="Saturación de oxígeno: {}%".format(sat)
                    enviar8="Presión arterial: {}".format(presion)
                    ...
                    enviar14="Examen físico (OBJETIVO): {}".format(examen)
                    enviar15="Diagnóstico (ANÁLISIS): {}".format(diag)
                    enviar16="Tratamiento (PLAN): {}".format(trat)
                    enviar17="Observaciones: {}".format(obs)
                    enviar18="Próxima revisión: {}".format(prox)
